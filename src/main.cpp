...
{
    {
        ...
        if (!pto->fDisconnect && state.vBlocksInFlight.size() > 0 && state.vBlocksInFlight.front().nTime < nNow - 500000 * ((pindexLast->nHeight <= Params.LastBootBlock()) ? Params().BootTargetSpacing() : Params().TargetSpacing()) * (4 + state.vBlocksInFlight.front().nValidatedQueuedBefore)) {
