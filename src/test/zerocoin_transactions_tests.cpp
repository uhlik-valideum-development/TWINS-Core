...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zvalideumTracker = unique_ptr<CzTFTracker>(new CzTFTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZVALIDEUM_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZVALIDEUM_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
