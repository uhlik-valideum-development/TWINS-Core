...
*Setup instructions for a gitian build of TF using a Debian VM or physical system.*

* this process is not yet implemented ! *

Gitian is the deterministic build process that is used to build the TF
...
Clone the git repositories for valideum and gitian and then checkout the valideum version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the valideum repository
...
To build TF (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the valideum repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/valideum/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/NewCapital/VALIDEUM-Core.git
...
For example:
```bash
URL=https://github.com/crowning-/valideum.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit valideum=${COMMIT} --url valideum=${URL} ../valideum/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit valideum=${COMMIT} --url valideum=${URL} ../valideum/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit valideum=${COMMIT} --url valideum=${URL} ../valideum/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/valideum-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/valideum-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/valideum-build.assert
```
...
[valideum/gitian.sigs](https://github.com/NewCapital/VALIDEUM-gitian.sigs/) repository, or if that's not possible to create a pull
request.
